...
FanActuator::FanActuator()
    : _pin(0), _relayOnLevel(LOW), _currentSpeed(0), _adaptiveMode(false)
    , _pwmActive(false), _adaptiveActive(false), _baseTemp(0), _baseHum(0)
    , _lastAdaptiveCheck(0), _startingPulseActive(false), _startingPulseStart(0) {
...
void FanActuator::init(uint8_t pin, uint8_t relayOnLevel, bool bootState, uint16_t defaultSpeed) {
    _pin = pin;
...
void FanActuator::update() {
    ...
    if (_adaptiveMode && getState() && config_get()->sensorControlMode && sensor_isOk()) {
        adaptiveUpdate();
    }
}
...
void FanActuator::adaptiveUpdate() {
    ...
    if (millis() - _lastAdaptiveCheck < config_get()->sensorInterval * 1000UL) return;
    _lastAdaptiveCheck = millis();
...
int FanActuator::calculateAdaptiveStep(float deltaTemp, float deltaHum, float humRate) {
    int step = ADAPTIVE_STEP_SIZE;
    if (deltaTemp > ADAPTIVE_EPSILON_TEMP * 2 || deltaHum > ADAPTIVE_EPSILON_HUM * 2) step *= 2;
    if (deltaTemp > ADAPTIVE_EPSILON_TEMP * 3 || deltaHum > ADAPTIVE_EPSILON_HUM * 3) step *= 3;
